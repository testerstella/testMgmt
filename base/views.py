...
def interface_update(request):
    if request.method == 'POST':
        ...
        Interface.objects.filter(if_id=if_id).update(if_name=if_name, url=url, project=project, method=method, data_type=data_type,
                              is_sign=is_sign, description=description, request_header_param=request_header_data1,
                              request_body_param=request_body_data1, response_header_param=response_header_data1,
    ...
    return render(request, "base/interface/update.html", {"it": it, "prj_list": prj_list, "request_header_dict":request_header_dict, "request_body_dict":request_body_dict,"response_header_dict":response_header_dict,"response_body_dict":response_body_dict})
...
def case_update(request):
    if request.method == 'POST':
        ...
        description = request.POST['description']
        content = request.POST['content']
        Case.objects.filter(case_id=case_id).update(case_name=case_name, project=project, content=content, description=description)
        return HttpResponseRedirect("/base/case/")
...
def plan_update(request):
    if request.method == 'POST':
        ...
        content = request.POST.getlist("case_id")
        description = request.POST['description']
        Plan.objects.filter(plan_id=plan_id).update(plan_name=plan_name, environment=environment, project=project, description=description, content=content)
        return HttpResponseRedirect("/base/env/")
